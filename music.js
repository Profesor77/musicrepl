...
setInterval(() => {
  http.get(`http://annta5.glitch.me/`); //تعديل مهم حط اسم المشروع
}, 280000);
...
const GUILDID = '732279241416245289'; //  تعديل مهم ايدي السيرفر  
const CHANNELID = ''; // تعديل مهم ايدي الروم الصوتي
...
client.on('ready', () => {
    client.user.setActivity(`${prefix}play`,{ type: '' });
})


//تغير الحالة
const developers = ["707128720980312146","","",""] ///تعديل مهم حط هنا ايدي اللي بدك يتحكمو بالحالة
client.on('message', message => {
    ...
    if (message.content.startsWith(prefix + 'setStreaming')) {
      ...
      message.delete();
      client.user.setGame(argresult, 'https://twitch.tv/Profesor555');
...
function play(guild, song) {
  ...
  if(serverQueue.repeating) {
	...
	dispatcher.setVolumeLogarithmic(serverQueue.volume / 150);
        ...
        if(!msg.member.hasPermission('SPEAK')) {
          msg.react('❌')
